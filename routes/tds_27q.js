...
// POST: Create new Form 27Q return
router.post('/api/tcs27q', async (req, res) => {
  const {
    collectorDetails,
    challanDetails,
    collecteeDetails,
  ...
  try {
    ...
    const [result] = await conn.query(
      `INSERT INTO tcs_27q_returns (
        ...
        responsible_person_flat_no, responsible_person_premises_name, responsible_person_road_street, responsible_person_area, responsible_person_town, responsible_person_state, responsible_person_pin_code, responsible_person_mobile_no, responsible_person_alternate_mobile, responsible_person_email,
    ...
    if (collecteeDetails.length > 0) {
      ...
      await conn.query(
        `INSERT INTO tcs_27q_collectees (
    ...
    return res.json({ success: true, message: 'Form 27Q saved successfully', returnId });
  } catch (err) {
    await conn.rollback();
    conn.release();
    console.error('Error in Form 27Q insert:', err);
    return res.status(500).json({ error: 'Failed to save Form 27Q', details: err.message });
  }
});

// GET: List returns by financial year and quarter (optional filters)
router.get('/api/tcs27q', async (req, res) => {
  const { year, quarter } = req.query;

  let sql = `
    SELECT r.id, r.tan, r.financial_year, r.quarter, r.collector_name, 
      COUNT(c.id) AS total_challans,
      COALESCE(SUM(c.total), 0) AS total_tax_deposited,
      COUNT(co.id) AS total_collectees,
      COALESCE(SUM(co.tax_collected), 0) AS total_tax_collected,
      r.created_at
    FROM tcs_27q_returns r
    LEFT JOIN tcs_27q_challans c ON r.id = c.return_id
    LEFT JOIN tcs_27q_collectees co ON r.id = co.return_id
    WHERE 1=1
  `;
  const params = [];
  if (year) {
  ...
  try {
    ...
    console.error('Error fetching Form 27Q returns:', err);
